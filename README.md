...
日本のレセプト電算マスターを、区分番号・レセ電コード・ICD-10などの異なる軸を混同せずに保持できるか検証したSQLite PoCです。理学療法領域を入口に、公式配布ファイルの取込、対応関係のモデル化、検証クエリ、静的HTMLでの表示を実装しています。

- 読み物：[レセプト電算マスターを読み解く](https://norio111.github.io/med-code-map/)
- ツール：[施設基準別の変更点チェック](https://norio111.github.io/med-code-map/impact.html)（令和8年度改定　施設基準別の変更点チェック）
...
「レセプト電算マスターを読み解く」として、取込結果の一部を公開しています（ソースは [`index.html`](index.html)）。

## 施設別の改定影響チェッカー（MVP）

impact.htmlでは、脳血管・廃用・運動器のⅠ・Ⅱ・Ⅲ（9区分）を選び、令和6年度から令和8年度への制度変更候補を確認できます。収録は8制度項目です。初期・急性期・データ提出加算のⅡ・Ⅲは要確認、総合計画評価料はⅢ単独では対象外です。旧「リ減」と新「特定患者」は別項目として扱います。この画面は変更点の確認を補助するもので、算定可否を判定するものではありません。

根拠ページ、年度別内容、点数・期間・コード構成、確認事項、選択区分別のコード明細を表示します。手入力した制度上の整理と、マスターから抽出した属性は分けて保持しています。
...
施設基準3件と比較改定が表示された後、施設基準を1つ以上選ぶと「変更点を表示」ボタンが有効になります。閲覧にSQLiteや公式CSVは不要です。

`file:///`ではJSONの`fetch()`が遮断されるため、両起動方法とも生成済みの`assets/impact-data.js`を通常のscriptタグで読み込みます。JavaScriptやデータが欠落・破損した場合は、画面内に原因と復旧・起動方法を表示します。JavaScript無効時もHTMLの案内が残ります。
...
公式資料との再照合で、旧版の誤りを2点修正しました。

s_20260501.csvの10,192件は令和8年度版ではなく、令和6年度版（2026年5月31日まで適用）のスナップショットでした。

傷病名マスターの項番22は「収載年月日」、項番23が「変更年月日」です。旧コードは項番22を変更年月日としていました。
...
差分の読み方についても、集計範囲の明示不足、点数変更の解釈、廃止コードと新設コードの対応づけを修正しました。修正の経緯は docs/revision-diff-R06-R08.md に残しています。
...
med-code-map/
├─ README.md
├─ index.html                 レセプト電算マスターを読み解く（読み物）
├─ impact.html                施設別の改定影響チェッカー
├─ assets/
│  ├─ impact.css
│  ├─ impact.js
│  ├─ impact-data.js         生成物（手編集しない）
│  └─ impact-data.json       生成物（手編集しない）
├─ src/
│  ├─ load_master.py         医科診療行為マスター取込
│  ├─ load_disease.py        傷病名・移行対応取込
│  ├─ load_gigi.py           疑義解釈取込
│  ├─ export_impact.py       チェッカー用データ生成
...
├─ docs/
│  ├─ design-notes.md
│  ├─ impact-design.md       チェッカーのデータモデル・収録範囲
...
├─ data/
│  ├─ README.md
│  └─ impact-catalog.json    チェッカーの監査用正本
└─ tests/
   ├─ test_smoke.py
   ├─ impact.test.cjs
   └─ browser_impact.py
